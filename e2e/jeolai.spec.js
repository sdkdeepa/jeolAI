...
async function mockApi(page) {
  ...
  let pendingSummitSize = false;
  let summitSize = null;
  await page.route('http://127.0.0.1:8000/chat', async route => {
    const body = route.request().postDataJSON();
    const message = body.message.trim();
    const blocked = /reverse|string|kubernetes|poem/i.test(message);
    trace = blocked ? blockedTrace : normalTrace;
    let reply = 'I found Summit GTX Hiking Boots for $159.';

    if (/add\s+summit\s+gtx/i.test(message)) {
      pendingSummitSize = true;
      reply = 'What size would you like? Available sizes are 6, 7, 8, 9, 10, 11, and 12.';
    } else if (pendingSummitSize && /^size\s+10$/i.test(message)) {
      pendingSummitSize = false;
      summitSize = '10';
      reply = 'Summit GTX Hiking Boots, size 10, was added to your cart.';
    } else if (/show me my cart/i.test(message)) {
      reply = summitSize
        ? `Your cart has Summit GTX Hiking Boots, size ${summitSize}.`
        : 'Your cart is empty.';
    }

    await route.fulfill({ status:200, contentType:'application/json', body:JSON.stringify(blocked ? { reply:"I don't know. I can only help with shopping, products, promotions, inventory, carts, and checkout.", domain_allowed:false, requires_approval:false } : { reply, domain_allowed:true, requires_approval:false }) });
  });
...
test('requires product variant before adding sized product to cart', async ({ page }) => {
  const input = page.getByPlaceholder(/Build me a hiking outfit/i);

  await input.fill('Add Summit GTX to my cart.');
  await page.getByRole('button', { name: 'Send' }).click();
  await expect(page.getByText(/What size would you like\?/i)).toBeVisible();
  await expect(page.getByText(/Summit GTX Hiking Boots, size 10, was added/i)).toHaveCount(0);

  await input.fill('Size 10');
  await page.getByRole('button', { name: 'Send' }).click();
  await expect(page.getByText(/Summit GTX Hiking Boots, size 10, was added/i)).toBeVisible();

  await input.fill('Show me my cart.');
  await page.getByRole('button', { name: 'Send' }).click();
  await expect(page.getByText(/Your cart has Summit GTX Hiking Boots, size 10/i)).toBeVisible();
});
